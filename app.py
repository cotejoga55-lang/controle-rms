...
# --- LÓGICA DE NAVEGAÇÃO (CASO SEPARAÇÃO) ---
    ...
    if aba_sep_sel == "📥 Estoque Geral":
        ...
        if st.button("Processar 10k Linhas"):
            if txt_input:
                linhas = [l.split('\t') for l in txt_input.split('\n') if l.strip()]
        ...
        if st.session_state['estoque_geral'].empty:
            st.error("Carregue o estoque na aba anterior primeiro.")
        else:
