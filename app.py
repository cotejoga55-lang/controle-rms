...
# Definição unificada e limpa das abas para evitar duplicidade estrutural no DOM
if es_admin:
    nomes_abas = ["📊 Dashboard", "📋 Painel", "📦 Pend. Retirada", "➕ Nova RM", "🔍 Consulta", "📊 Histórico"]
else:
    nomes_abas = ["📋 Painel", "📦 Pend. Retirada", "🔍 Consulta", "📊 Histórico"]

tabs = st.tabs(nomes_abas)
abas_dict = dict(zip(nomes_abas, tabs))

if "📊 Dashboard" in abas_dict:
    with abas_dict["📊 Dashboard"]:
...
if "📋 Painel" in abas_dict:
    with abas_dict["📋 Painel"]:
...
if "📦 Pend. Retirada" in abas_dict:
    with abas_dict["📦 Pend. Retirada"]:
...
if "➕ Nova RM" in abas_dict:
    with abas_dict["➕ Nova RM"]:
...
if "🔍 Consulta" in abas_dict:
    with abas_dict["🔍 Consulta"]:
        ...
        if st.button("Pesquisar", key="btn_pesquisa_rm"):
            if busca:
...
if "📊 Histórico" in abas_dict:
    with abas_dict["📊 Histórico"]:
        st.markdown("<h3 style='text-align: center;'>📊 Histórico Completo</h3>", unsafe_allow_html=True)
        col_c = st.columns([1, 8, 1])[1]
        with col_c:
            # CORREÇÃO CRÍTICA: Utilizando componente nativo do Streamlit para evitar quebra de layout HTML
