...
with st.sidebar:
    ...
    aba_sep = st.radio("Selecione:", ["📥 Entrada", "📑 Gerar Picklist"], key="nav_sep")
...
# 1. ABA DE RM's (CÓDIGO ORIGINAL PRESERVADO)
if st.session_state.nav_rm:
    ...
    if aba_selecionada == "📊 Dashboard":
        ...
        if df_painel.empty:
            st.info("Nenhuma RM em aberto ou em separação no momento.")
...
# 2. ABA DE SEPARAÇÃO (NOVA ESTRUTURA)
elif st.session_state.nav_sep:
    aba_sep = st.session_state.nav_sep
    st.title(f"🚀 Controle de Separação: {aba_sep}")
    st.divider()
    if aba_sep == "📥 Entrada":
        st.info("Cole abaixo a sua demanda (Partnumber e Quantidade):")
        st.text_area("Dados da Demanda (Ctrl+V aqui):", height=200)
    elif aba_sep == "📑 Gerar Picklist":
        st.write("Aguardando lógica de estoque...")
