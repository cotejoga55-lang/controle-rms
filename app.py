...
df["data_retirada"] = pd.to_datetime(
    df["data_retirada"],
    errors="coerce"
)
...
with st.sidebar:
    st.markdown(
        f"### 👤 Perfil: **{st.session_state['perfil_logado']}**"
    )
...
if st.session_state.get("nav_rm"):
    ...
    if aba_selecionada == "📊 Dashboard":
        avisos = df[
            df["cobranca"].str.contains(
                "está cobrando|Comentario adicionado",
                case=False,
                na=False
            )
        ]
        ...
        if not avisos.empty:
            with st.popover(f"🔔 NOTIFICAÇÕES ({len(avisos)})"):
                for _, row in avisos.iterrows():
                    st.warning(
                        f"{row['cobranca']} na RM {row['numero_rm']}!"
                    )

                    if st.button(
                        f"Limpar {row['numero_rm']}",
                        key=f"clr_ntf_{row['id']}"
                    ):
                        sheet.update_cell(
                            sheet.find(
                                str(row["id"]),
        ...
        opcoes = [
            f"{nomes_meses[mes.month].capitalize()} - {mes.year}"
        ...
        if mes_escolhido:
            ...
            c_r1.metric(
                ...
                len(
                    df[
                        (df["data_entrada"].dt.month == m_num)
                        & (df["data_entrada"].dt.year == ano)
                    ]
                )
            )
            ...
            c_r2.metric(
                ...
                len(
                    df[
                        (df["data_retirada"].dt.month == m_num)
                        & (df["data_retirada"].dt.year == ano)
                    ]
                )
            )
        ...
        if df_painel.empty:
            st.info(
                "Nenhuma RM em aberto ou em separação no momento."
            )
        ...
        for _, row in df_painel.iterrows():
            with st.expander(
                f"RM: {row['numero_rm']} - "
                f"{row['solicitante']} | "
                f"{formatar_status_tempo(row['data_entrada'], row['status'])}"
            ):
                ...
                with b1:
                    with st.popover("🔔 Cobrar"):
                        nome = st.text_input(
                            "Quem está cobrando?",
                            key=f"cob_txt_{row['id']}"
                        )

                        if st.button(
                            "Confirmar",
                            key=f"btn_cob_{row['id']}"
                        ):
                            sheet.update_cell(
                                sheet.find(
                                    str(row["id"]),
        ...
        if df_pendente.empty:
            st.info("Nenhuma RM pendente de retirada.")

        for _, row in df_pendente.iterrows():
            tempo_decorrido = (
                datetime.now()
                - pd.to_datetime(row["data_saida"])
            )
            is_atrasado = (
                tempo_decorrido
                > timedelta(hours=72)
            ...
            if is_atrasado:
                label = "🔴 Separada a mais de 72 horas"
            else:
                label = "🟢 Separada recentemente"

            with st.expander(
                f"RM: {row['numero_rm']} - "
                f"{row['solicitante']} | {label}"
            ):
                if is_atrasado:
                    st.error(label)
                else:
                ...
                if es_admin:
                    with st.form(
                        key=f"ret_form_{row['id']}"
                    ):
                        quem = st.text_input(
                            "Quem retirou?",
                            key=f"quem_in_{row['id']}"
                        )
        ...
        busca = st.selectbox(
            ...
            options=lista_rms,
            index=None,
            placeholder="Digite ou selecione a RM...",
            key="sb_pesquisa_rm"
        )
        ...
        if st.button(
            ...
            if busca:
                st.session_state["ultima_busca_rm"] = (
                    str(busca).strip()
                )

        if (
            "ultima_busca_rm" in st.session_state
            and st.session_state["ultima_busca_rm"]
        ):
            res = df[
                df["numero_rm"].astype(str)
                == st.session_state["ultima_busca_rm"]
            ]
            ...
            if not res.empty:
                ...
                with st.container(border=True):
                    ...
                    with c1:
                        ...
                        if pd.notna(rm.get("data_entrada")):
                            st.markdown(
                                "**📅 Data:** "
                                f"{pd.to_datetime(rm['data_entrada']).strftime('%d/%m/%Y %H:%M')}"
                            )
        ...
        if es_admin:
            with st.form("form_del_hist"):
                ...
                if st.form_submit_button(
                    ...
                    for registro_id, selecionado in selecionadas.items():
                        if selecionado:
                            sheet.delete_rows(
                                sheet.find(
                                    str(registro_id),
